...
bool isCalibConfigured = false;
std::ofstream calibTrackerFile;
...
void SaveCalibData()
{
    ...
    if (isCalibConfigured == false) {
        ...
        captureSetColorFormat(STREAM_DONT_CAPTURE);
...
void XtionConfig()
{
    ...
    extern openni::VideoStream g_colorStream;

    if (g_colorStream.getCameraSettings() != NULL) {
        g_colorStream.getCameraSettings()->setAutoExposureEnabled(false);
		g_colorStream.getCameraSettings()->setAutoWhiteBalanceEnabled(false);

		g_colorStream.getCameraSettings()->setGain(350);
    }
